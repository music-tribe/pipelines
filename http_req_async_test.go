...
func Test_httpReqAsyncWithHeartbeats(t *testing.T) {
	t.Parallel()
	...
	t.Run("Should return a channel with no error & response", func(t *testing.T) {
		...
		taskThatNeedsAHeartbeat := func(ctx context.Context) HttpReqAsyncResponse[sampleObject] {
			...
			return <-HttpReqAsync(ctx, client, req, resHandler)
		}
	...
	t.Run("Should receive an error when the function times out", func(t *testing.T) {
		ctx := context.TODO()
		expectedObject := createSampleObject()
		...
		assert.Equal(t, HttpReqAsyncResponse[sampleObject]{}, res)
	})
}
